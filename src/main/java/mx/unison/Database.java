...
public class Database {
    // [BUG-FIX #1] Ruta dinámica y relativa en lugar de la ruta preestablecida por el dev junior
    // (V4: Eliminación de la columna 'departamento' para arreglar Bug #7)
    private static final String URL = "jdbc:sqlite:InventarioV4.db";
    ...
    private void init() {
        try (Connection c = connect(); Statement st = c.createStatement()) {
            // [BUG-FIX #2] Tabla usuarios (Corregido 'nombre TEXT PRIMARY KEY' por 'nombre TEXT UNIQUE' ya que no puede haber dos llaves primarias)
            st.execute(
                    "CREATE TABLE IF NOT EXISTS usuarios (id INTEGER PRIMARY KEY AUTOINCREMENT, nombre TEXT UNIQUE, password TEXT NOT NULL, fecha_hora_ultimo_inicio TEXT, rol TEXT NOT NULL)");
            // Tabla almacenes
            st.execute(
                    "CREATE TABLE IF NOT EXISTS almacenes (id INTEGER PRIMARY KEY AUTOINCREMENT, nombre TEXT NOT NULL, ubicacion TEXT, fecha_hora_creacion TEXT, fecha_hora_ultima_modificacion TEXT, ultimo_usuario_en_modificar TEXT)");
            // [BUG-FIX #3 y #7] Se renombró almacen a almacen_id y se eliminó 'departamento TEXT NOT NULL' ya que el modelo nunca lo usa
            st.execute(
                    "CREATE TABLE IF NOT EXISTS productos (id INTEGER PRIMARY KEY AUTOINCREMENT, nombre TEXT NOT NULL, precio REAL DEFAULT 0.0, cantidad INTEGER NOT NULL, almacen_id INTEGER, descripcion TEXT, fecha_hora_creacion TEXT, fecha_hora_ultima_modificacion TEXT, ultimo_usuario_en_modificar TEXT)");
            ...
            // Establecer fecha de creación para productos/almacenes existentes si están
            // vacíos
        ...
        try (Connection c = connect(); PreparedStatement ps = c.prepareStatement(check)) {
            ...
            if (!rs.next()) {
                ...
                try (PreparedStatement ps2 = c.prepareStatement(ins)) {
                    ...
                    ps2.setString(3, rol);
                    // [BUG-FIX #4] Ejecutar la consulta SQL (el desarrollador olvidó ejecutar el PreparedStatement)
                    ps2.executeUpdate();
                }
            }
        }
