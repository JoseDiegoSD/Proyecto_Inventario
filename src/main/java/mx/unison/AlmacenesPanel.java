...
public class AlmacenesPanel extends JPanel {
    ...
    private void initTop() {
        ...
        edit.addActionListener(e -> {
            ...
            if (r >= 0) {
                ...
                a.id = id;
                a.nombre = (String) model.getValueAt(r, 1);
                a.ubicacion = (String) model.getValueAt(r, 2);
                openForm(a);
            }
        });
    ...
    private void openForm(Almacen a) {
        ...
        FormAlmacen dialog = new FormAlmacen(owner, a);
        dialog.setVisible(true);
        if (dialog.isSaved()) {
            if (a == null) {
                db.insertAlmacen(dialog.getNombre(), dialog.getUbicacion(), "ADMIN");
            } else {
                db.updateAlmacen(a.id, dialog.getNombre(), dialog.getUbicacion(), "ADMIN");
            }
        }
        loadData();
    }
